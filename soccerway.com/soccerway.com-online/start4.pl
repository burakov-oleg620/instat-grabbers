...
my $lockfile_graber = getcwd .'/lock_graber.txt'; 
	...
	if (scalar (@url) > 0) {
		...
		if (scalar (@file1) > 0) {
			...
			foreach (@file1) {
				...
				chdir ($workdir1); 
				my $file_url = $workdir1  .'/xls2/001.xls';
				...
				# my $path  = '/usr/local/bin/perl '.$workdir1 .'/'. $file2;
				# print '<br />' ."\n";
				# print $path ."\n";
				...
				if (-f $workdir1 .'/'. $file2 and !-d 'c:/windows') {
					...
					# my $path  = '/usr/local/bin/perl '.$workdir1 .'/'. $file2;
					# my $path = '/usr/local/bin/php '.$path;	
					# my $path  = '/usr/local/bin/php ./test.php';
					
					my $path  = '/usr/local/bin/perl ./start3.pl';
