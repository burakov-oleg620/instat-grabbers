...
while (my $str1 = $read_text_file1 -> get_str ()) {
	...
	if ($str1 =~ /\t/) {
		...
		if (scalar (@$temp1 > 1)) {			
			...
			$insert -> {id} = $temp1 -> [1];
			$insert -> {name} = $temp1 -> [2];
			$insert -> {address} = $temp1 -> [3];
			$insert -> {zip_code} = $temp1 -> [4];
			$insert -> {city} = $temp1 -> [5];
			$insert -> {opened} = $temp1 -> [6];
			$insert -> {architect} = $temp1 -> [7];
			$insert -> {capacity} = $temp1 -> [8];
			$insert -> {surface} = $temp1 -> [9];
			$insert -> {surface} = $temp1 -> [9];
			$insert -> {previous_names} = $temp1 -> [10];
			$insert -> {facts} = $temp1 -> [11];
			$insert -> {lat} = $temp1 -> [12];
			$insert -> {lon} = $temp1 -> [13];
			$insert -> {phone} = $temp1 -> [14];
			$insert -> {fax} = $temp1 -> [15];
			$insert -> {img} = $temp1 -> [16];
			$insert -> {import_date} = $temp1 -> [17];
			
			
			
			$insert -> {stadium_name} = $temp1 -> [18];
			$insert -> {stadium_id} = $temp1 -> [19];
...
sub post {
    ...
    my $url = 'http://data.instatfootball.tv/api/import2?key='.$key_import2.'&method=UpdateScoreswayStadiums';	
