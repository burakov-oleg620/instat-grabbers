...
my $write_file1 = $workdir2.'/write_text_file_mode_rewrite2.xls'; 
...
while (my $file1 = $dir_scan_recurce -> get_file ()) {
	...
	if ($file1 =~ /$pattern/) {	
						...
						if (defined $work_for_content_result -> [0]) {
							foreach (@$work_for_content_result) {		
								...
								if ($_ =~ /$pattern1/) {
									...
									#сначала берем всю ссылку , но там часто режут
									my $pattern1 = '^(.+?)$';
									my $work_for_content = work_for_content -> new ($a); 
									my $work_for_content_result = $work_for_content -> get_pattern ($pattern1); 
									if (defined $work_for_content_result -> [0]) {
										foreach (@$work_for_content_result) {		
											my $clear_str = clear_str -> new ($_);
									...
									#в хинт на ссылке всплывает полное
									$pattern1 = 'title="(.+?)"';
									$work_for_content = work_for_content -> new ($a); 
									$work_for_content_result = $work_for_content -> get_pattern ($pattern1); 
									if (defined $work_for_content_result -> [0]) {
										foreach (@$work_for_content_result) {		
											my $clear_str = clear_str -> new ($_);
		...
		if (scalar (@array) > 0) {
			...
			if (exists ($file1 {$array[0]})) {
				
				# $write_to_txt1 -> put ($file1 {$array[0]} -> [2]);
				# $write_to_txt1 -> put ($file1 {$array[0]} -> [3]);
				
				$write_to_txt1 -> put ($file1 {$array[0]} -> [12]);
				$write_to_txt1 -> put ($file1 {$array[0]} -> [13]);
