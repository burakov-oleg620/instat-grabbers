...
while (my $str1 = $read_text_file1 -> get_str ()) {
	...
	if ($str1 =~ /\t/) {
		...
		if (scalar (@$temp1 > 1)) {			
			...
			$insert -> {height} = $temp1 -> [10];
			$insert -> {weight} = $temp1 -> [11];
			$insert -> {img} = $temp1 -> [12];
			$insert -> {dead} = $temp1 -> [13];
...
sub post {
    ...
    my $url = 'http://data.instatfootball.tv/api/import2?key='.$key_import2.'&method=UpdateFootballScoreswayCoaches';	
...
------------------------------4b5789fa8d3f--';
	...
	my $file = getcwd () .'/txt/coaches.html';
