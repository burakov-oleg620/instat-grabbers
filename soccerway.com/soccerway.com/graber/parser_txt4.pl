...
my $file1 = $workdir1.'/write_text_file_mode_rewrite5.xls'; 
...
while (my $str1 = $read_text_file1 -> get_str ()) {
	...
	if ($str1 =~ /\t/) {
		...
		if (scalar (@$temp1 > 1)) {			
			...
			$insert -> {id} = $temp1 -> [1];
			$insert -> {name} = $temp1 -> [2];
			$insert -> {surname} = $temp1 -> [3];
			$insert -> {birthday} = $temp1 -> [4];
			$insert -> {age} = $temp1 -> [5];
			$insert -> {country} = $temp1 -> [6];
			$insert -> {place_of_birth} = $temp1 -> [7];
			$insert -> {nationality} = $temp1 -> [8];
			$insert -> {height} = $temp1 -> [9];
			$insert -> {weight} = $temp1 -> [10];
			$insert -> {img} = $temp1 -> [11];
			$insert -> {dead} = $temp1 -> [12];
