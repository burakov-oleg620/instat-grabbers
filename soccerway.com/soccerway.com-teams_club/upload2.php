...
//http://data.instatfootball.tv/parser_short_data/parser_matches_xml_int_soccerway_com3/upload.php?date=2019-08-31&id=3155357
//http://test9.ru/soccerway.com/upload2.php?date=2021-12-07&id=3633458
...
if (
	...
	$workdir1 = getcwd ();
	//$workdir2 = $workdir1.'/graber/xls2';
	$workdir2 = $workdir1.'/city';
	$file2 = $workdir2 .'/001.xls';
	
	$content = 'https://int.soccerway.com/matches/'.$date_a[0].'/'.$date_a[1].'/'.$date_a[2].'/1/1/1/1/'.$id.'/'."\t".$date."\t".$id;
	...
	// system ('ls');
	//print getcwd () ."\n".'<br />';
	$path = getcwd () .'/start4.pl';
	// $path = getcwd () .'/test.php';
	 //$path = getcwd () .'/p.pl';
	...
	if (is_file ($path)) {
		...
		if (is_file ($path)) {
			...
			// if (is_dir ('c:/windows')) {
				// $xml_path = 'http://test9.ru/soccerway.com/graber/out/'.$id.'.xml';
			// } else {
				// $xml_path = 'http://data.instatfootball.tv/parser_short_data/xml/'.$id.'.xml';
			// }
			
			// print '<script>';
			// print 'document.location.href = "'.$xml_path.'"';
			// print '</script>';
