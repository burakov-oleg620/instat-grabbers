	...
	if ($str1 =~ /\t/) {
		...
		if (scalar (@$temp1 > 1)) {			
			...
			if ($temp1 -> [0] ne '-') {
						...
						foreach (@content) {				
							...
							if (defined $work_for_content_result -> [0]) {
								foreach (@$work_for_content_result) {		
									...
									$insert -> {match_date} = $insert -> {match_date} . ' '.$_;
									$_ = entities_encode ($_);
									$time = $_;
								}
							}
				...
				#xml формируем только если есть составы
				if ($temp1 -> [16] ne '-') {
				...
				if ($temp1 -> [31] ne '-') {
					$insert -> {details} = $temp1 -> [31];
				}
				
				# $insert -> {import_date} =  get_date ();
				# $insert -> {date} =  get_date ();

				$insert -> {import_date} =  get_date2 (time ());
				$insert -> {date} =  get_date2 (time ());
				
				
				if ($temp1 -> [32] ne '-') {
					
					my $md5 = MD5 -> new ();
					my $hash = $md5 -> hexhash ($temp1 -> [32]);
					$md5 = undef;

					$insert -> {stadium_name} = $temp1 -> [32];
					$insert -> {stadium_id} =  ($hash);
				}
...
sub post {
    ...
    #my $url = 'http://data.instatfootball.tv/api/import2?key='.$key_import2.'&method=UpdateFootballScoreswayMatches';
    my $url = 'https://data.instatfootball.tv/api/import2?key='.$key_import2.'&method=UpdateFootballScoreswayMatches';
...
sub get_date2 { 
	...
	$time = $time + 3*3600;
	my ($year,$month,$day, $hour,$min,$sec) = Time_to_Date($time);
	my $str = $year .'-'.good_count2($month).'-'.good_count2($day) .' '.good_count2($hour).':'.good_count2($min).':'.good_count2($sec);
	return $str;
}
