	...
	if (preg_match ('/\t/', $str)) {
		...
		if (count ($temp1) > 1) {
			...
			if ($temp1[3] != '-') {
				...
				if ($picture != '-') {
					$picture_array = preg_split ('/\|\|/', $picture);
					foreach ($picture_array as $picture_array_key => $picture_array_value) {
						
						$picture_array [$picture_array_key] = replace ('^\s+','', $picture_array [$picture_array_key]);
						$picture_array [$picture_array_key] = replace ('\s+$','', $picture_array [$picture_array_key]);
						$picture_array [$picture_array_key] = replace ('\s+',' ', $picture_array [$picture_array_key]);
...
function post ($postdata) {
	...
	$url = 'http://data.instatfootball.tv/api/import2?key='.$key_import2.'&method=UpdateBasketballScoreswayMatches';
