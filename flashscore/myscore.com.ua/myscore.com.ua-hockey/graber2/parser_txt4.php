	...
	if (preg_match ('/\t/', $str)) {
		...
		if (count ($temp1) > 1) {
			...
			if ($temp1[0] != '-') {
				...
				//$insert ['date_of_birth'] = $temp1[12];
				$insert ['actual_date'] = $temp1[17];
...
function post ($postdata) {
	...
	$url = 'http://data.instatfootball.tv/api/import2?key='.$key_import2.'&method=UpdateBaseballScoreswayReferees';
	...
	if ($httpcode == 200) {
		...
		$file = $workdir .'/txt/post_json_referees.html';
